...
export class clear extends exec {
    ...
    constructor(target: (value: SetStateAction<HistoryItem[]>) => void, input: (value: SetStateAction<string>) => void) {
        super();
        this.target = target;
        this.input = input;
    }
    ...
    run(_args: string[]): commandOutput {
        ...
        return new commandOutput(ExitCode.EXIT_SUCCESS, "[clear] : console was cleared");
    }
...
export class help extends exec {
    ...
    run(_args: string[]): commandOutput {
        return new commandOutput(ExitCode.EXIT_SUCCESS, `Available Commands : ${this.context.commandRegistry.keys().toArray().join(", ")}.`)
    }
...
export class alias extends exec {
    ...
    protected parse(args: string[]): void {
        ...
        if (this.aliasName === "" || this.command === "") throw "Insufficient arguments.";
    }

    run(args: string[]): commandOutput {
        try { this.parse(args) } catch (e) {
        ...
        if (!commandClass || !this.aliasName) {
            return new commandOutput(ExitCode.EXIT_FAILURE, null, `[alias] unknown command ${this.command}`)
        }
...
export class vim extends exec {
    protected parse(_args: string[]): void { }
    run(_args: string[]): commandOutput {
        return new commandOutput(ExitCode.EXIT_FAILURE, null,
...
export class sudo extends exec {
    ...
    run(args: string[]): commandOutput {
        console.log(args);
        if (args.length <= 1 && args[0] === "") return new commandOutput(ExitCode.EXIT_FAILURE, null, "usage: sudo <command>");
