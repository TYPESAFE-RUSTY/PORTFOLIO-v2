...
export default function Terminal() {
  ...
  // states
  const [contentHistory, setContentHistory] = useState<HistoryItem[]>(["splash"]);
  ...
  const [terminal] = useState<fakeTerminal>(() => {
    ...
    // set commands
    _fakeTerminal.registerCommand(CLEAR, () => new clear(setContentHistory, setInput));
  ...
  useEffect(() => {
    ...
    setContentHistory(content => [...content, {
  ...
  function submitHandler(e: FormEvent<HTMLFormElement>) {
    ...
    let currentItem: HistoryItem = {
      ...
      cwd: pwd,
      command: input,
      output: result.exitCode() === ExitCode.EXIT_SUCCESS ? result.stdout() : result.stderr(),
      isMarkdown: cleanInput.startsWith("cat") && cleanInput.includes(".md")
    };


    const uiCommand = UI_COMMANDS.map(command => cleanInput.split(" ")[0].includes(command)).reduce((p, n) => p || n);
    if (!uiCommand) {
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Tab") {
      ...
      if (!isCyclingRef.current) {
        ...
        currentIndexRef.current = (currentIndexRef.current + 1) % completions.length;
        tokens.pop();
    ...
    >
      {
        ...
        contentHistory.map(element => {
          if (element === "splash") {
            return <SplashScreen key="splash-screen" commands={terminal.getRegisteredCommands()} />
          }

          if (element.type === ExitCode.EXIT_SUCCESS) {
            return <CommandOutput userName={USER_NAME} key={element.id} cwd={element.cwd} command={element.command}>
              {
                element.isMarkdown ? <Markdown>{element.output}</Markdown> : <pre className=" text-wrap">{element.output}</pre>
              }
            </CommandOutput>

          } else {
            return <Error userName={USER_NAME} key={element.id} cwd={element.cwd} command={element.command} help={false}>
